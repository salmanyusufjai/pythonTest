...
class JsonEnricher:
    def __init__(self, input_file, reference_file, output_dir):
        self.input_file = input_file
        self.reference_file = reference_file
        self.output_dir = output_dir

    def enrich_and_create_files(self):
        # Load the reference JSON data from the reference file
        '''with open(self.reference_file, 'r') as reference_file:
            reference_data = json.load(reference_file)'''

        # Read the input JSON data into a pandas DataFrame
        input_df = pd.read_json(self.input_file)

        # Initialize an empty list to store enriched data
        enriched_data = []
    ...
    def enrich_row(self, row, reference_file):
        ...
        print(newRow)
        if not result.empty:
          # Select the first matching row (index 0) and convert it to a dictionary
        ...
        # Implement your logic to enrich the row using reference_data
        # Return the enriched row as a dictionary

    def create_json_file(self, data, idx):
        # Create a JSON file for the enriched data``
